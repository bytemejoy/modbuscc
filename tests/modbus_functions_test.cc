#include "src/modbus_functions.h"
...
TEST(ModbusFunctionsTest, ReadCoils_Success) {
  ...
  EXPECT_CALL(*mock_client,
              SendReceive(0x11, FunctionCode::kReadCoils,
                          (std::vector<uint8_t>{0x00, 0x13, 0x00, 0x25})))
      .WillOnce(testing::Return(
          std::vector<uint8_t>{0x05, 0xCD, 0x6B, 0xB2, 0x0E, 0x1B}));

  auto result = ReadCoils(mock_client.get(), 0x11, 0x0013, 0x0025);
  std::cout << "SommerkDbg: " << result.status() << std::endl;
...
//       .WillOnce(
//           testing::Return(std::vector<uint8_t>{0x04, 0x13, 0x88, 0x00,
//           0x00}));
...
//       .WillOnce(testing::Return(std::vector<uint8_t>{0x00, 0x13, 0xFF,
//       0x00}));
...
//       .WillOnce(testing::Return(std::vector<uint8_t>{0x00, 0x02, 0x13,
//       0x88}));
...
//                   (std::vector<uint8_t>{0x00, 0x0F, 0x00, 0x05, 0x01,
//                   0xA5})))
//       .WillOnce(testing::Return(std::vector<uint8_t>{0x00, 0x0F, 0x00,
//       0x05}));
...
//       .WillOnce(testing::Return(std::vector<uint8_t>{0x00, 0x01, 0x00,
//       0x02}));

//   std::vector<uint16_t> registerValues{10, 4000};
//   auto result = WriteMultipleRegisters(mock_client.get(), 1, 1,
//   registerValues); ASSERT_TRUE(result.ok());
// }
...
//   std::vector<uint16_t> registerValues{10, 4000};
//   auto result = WriteMultipleRegisters(mock_client.get(), 1, 1,
//   registerValues); ASSERT_FALSE(result.ok()); ASSERT_EQ(result.code(),
